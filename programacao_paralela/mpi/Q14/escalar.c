...
int main(int argc, char *argv[])
{
    ...
    if (my_rank == 0)
    {
        ...
        for (i = 0; i < size_array; i++)
            produto_seq += (long long int)array_a[i] * array_b[i];
    }
    ...
    if (my_rank == 0)
    {
        printf("Produto escalar entre A e B (paralelo): %lld\n", local_prod_esc);
        printf("Produto escalar sequencial:              %lld\n", produto_seq);
        if (local_prod_esc == produto_seq)
            printf("Verificação OK: resultados coincidem.\n");
        else
